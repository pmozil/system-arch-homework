...
def get_service_url(service_name: str):
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(status_code=500, detail=f"Failed to fetch {service_name} instances from config server")
        instances = [ServiceInstance(**inst) for inst in response.json()]
        if not instances:
            raise HTTPException(status_code=500, detail=f"No available instances for service: {service_name}")
        instance = random.choice(instances)
        return f"http://{instance.ip}:{instance.port}"
    except requests.RequestException as e:
        raise HTTPException(status_code=500, detail=f"Error communicating with config server: {str(e)}")
...
async def send_message(message_request: MessageRequest):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise HTTPException(status_code=500, detail="Failed to forward message to logging service")
    except requests.RequestException as e:
...
async def get_messages():
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(status_code=500, detail="Failed to retrieve messages from logging service")
        ...
        return {"messages": {"logging_service": response.json(), "kafka_messages": kafka_response.json()}}
